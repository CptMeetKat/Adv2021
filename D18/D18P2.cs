...
public class D18P2 : AocMachine 
{
 
   List<char> snail = new List<char>();
   public D18P2(string filename) : base(filename){  }   
   ...
   private void calculateResult()
   {
      ...
      foreach(char c in snail)
      {
         if(c != ']')
            ...
            result = consolidate(stack);            
      }
      this.result = Math.Max(this.result, result);

   }
   private int consolidate(Stack<char> stack)
   {
      char c = stack.Pop();
      string temp = "";
      while(c != '[')
   ...
   private void findBestSnailNumber()
   {
