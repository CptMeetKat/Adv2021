...
public class D14P1 : AocMachine 
{
   ...
   public D14P1(string filename, int runs = 10) : base(filename)
   {
   ...
   private void findOptimalPolymerFormula1()
   {
      ...
      for (int i = 0; i < runs; i++)
      {
         ...
         evolved += polymer[polymer.Length-1];   //add last letter
         polymer = evolved;
         Console.WriteLine("Run: {0} {1}", i, polymer.Length);
         // Console.WriteLine("Pol: {0}", polymer);
         aggregateOccurances();
         printOccurances();
         occurances.Clear();
